...
k <- data.frame(pvalue=NA)
k[1:10,] <- NA
k[2,1] <- dfp(anova(fit1.2)$'Pr(>F)'[1])
k[10,1] <- dfp(anova(fit1.2)$'Pr(>F)'[2])

p <- dfp(summary(fit1.2)$coef[, 4])
x <- as.data.frame(cbind(round(summary(fit1.2)$coef[, 1],3), p))
y <- cbind(x,k)
colnames(y) <- c("Estimate", "p.t.test", "p.anova")
y <- y[2,]
knitr::kable(y)

t <- data.frame(pvalue=NA)
t[1:10,] <- NA
t[2,1] <- dfp(anova(fit2.2)$'Pr(>F)'[1])

p <- dfp(summary(fit2.2)$coef[, 4])
x <- as.data.frame(cbind(round(summary(fit2.2)$coef[, 1],3), p))
y <- cbind(x,t)
colnames(y) <- c("Estimate", "p.t.test", "p.anova")
y <- y[2,]
knitr::kable(y)
